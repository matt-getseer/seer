...
async function getOrCreateUnassignedTeam(
  prismaInstance: PrismaClient, // Use PrismaClient type
  organizationId: string,
  userId: number // Needed to associate the team if created by a user initially
): Promise<{ id: number }> { // Return type should match Team structure or just id
...
const updateTeam = async (req: AuthenticatedRequest, res: Response): Promise<Response | void> => {
  try {
    ...
    const updateData: { name?: string; department?: string } = {};
...
const deleteTeam = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    await typedPrismaClient.$transaction(async (tx: any) => {
...
const getTeamEmployeeCount = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
...
const createTeam = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    if (!req.user?.userId) {
      res.status(401).json({ error: 'User not authenticated' });
      return;
...
router.get('/:teamId/employee-count', authenticate, extractUserInfo, getTeamEmployeeCount);
