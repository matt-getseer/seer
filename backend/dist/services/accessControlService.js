...
async function getAccessibleTeams(currentUser) {
    if (currentUser.role === client_1.UserRole.ADMIN) {
        if (!currentUser.organizationId)
            return []; // Admin must belong to an org
    ...
    if (currentUser.role === client_1.UserRole.MANAGER) {
        ...
        let teamIdsFromHeadedDepartments = [];
    ...
    if (currentUser.role === client_1.UserRole.USER) {
        if (!currentUser.organizationId)
            return []; // User must belong to an org
...
async function getAccessibleMeetings(currentUser) {
    ...
    if (currentUser.role === client_1.UserRole.MANAGER) {
        ...
        let departmentIdsHeadedByManager = [];
        const headedDepartments = await prisma.department.findMany({
            where: { headId: currentUser.id, organizationId: currentUser.organizationId },
            select: { id: true }
        });
        ...
        let accessibleManagerUserIds = new Set(managerUserIdsInHierarchy);
        ...
        let accessibleEmployeeIds = new Set();
...
async function getAccessibleDepartments(currentUser) {
    if (!currentUser.organizationId)
        return []; // All roles need an org for this function
    if (currentUser.role === client_1.UserRole.ADMIN) {
    ...
    if (currentUser.role === client_1.UserRole.MANAGER) {
        ...
        const managerUserIdsInHierarchy = await getManagerialHierarchy(currentUser.id);
        let departmentIdsFromManagedTeams = [];
        if (managerUserIdsInHierarchy.length > 0) {
